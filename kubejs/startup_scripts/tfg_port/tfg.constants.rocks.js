...
// [PORT] TFG 1.20.1 startup_scripts/tfg/stone_types/constants.rocks.js -> 1.21.1 (Gregnautics).
// [PORT] Данные таблицы портированы без изменений: это только строки-ID, регистрирующие файлы
// [PORT] (tfg.blocks.rocks.js / tfg.items.rocks.js) фильтруют по префиксам 'tfg:rock/', 'tfg:loose/', 'tfg:brick/'.
// [PORT] ID отсутствующих модов (ad_astra, beneath, betterend) оставлены как данные — блоки этих модов
// [PORT] не регистрируются нами; tfg:-блоки космических пород (Ф10), чьи текстуры указывают на ad_astra/betterend,
// [PORT] получат missing texture до установки этих ресурсов/Ф10.
// [PORT] Требует load-order: createdeco.constants.js (priority 0) -> этот файл (priority -1) — global.CREATE_DECO_BRICK_TYPES
// [PORT] используется на верхнем уровне ниже.
...
console.info('[Gregnautics] progress: tfg_port rocks constants loaded (' + Object.keys(global.BIG_ROCK_TABLE).length + ' rock types)')
