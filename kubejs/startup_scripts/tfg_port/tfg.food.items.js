...
// [PORT] Из TFG startup_scripts/tfg/food/items.food.js (registerTFGFoodItems).
// Диспетчер Ф2 отключён — регистрация напрямую через StartupEvents.registry('item').
// [PORT] KubeJS 7: food.hunger() -> food.nutrition().
...
StartupEvents.registry('item', event => {
	...
	global.TFG_CREATE_GENERIC_FOOD_ITEM.forEach(foodItem => {
		let a = event.create(foodItem.id)
			...
			if (!foodItem.inedible) {
				a.food(food => {
					food.nutrition(1).saturation(1) // [PORT] hunger() -> nutrition()
	...
	// [PORT-CHECK] Джемы (tfg:jar/<fruit> и tfg:jar/<fruit>_unsealed) пропущены:
	// в kubejs_tfc 2.0.1 нет типа 'tfc:jar' (TFC 1.21 переработал банки),
	// а .placedModel/.modelJson/.color(layer, color) — API KubeJS 6.
	// Вернуться, когда kubejs_tfc добавит jar-билдер или банки будут решены иначе.
	/*
	...
	*/
	...
	event.create('tfg:used_foil_pack')
		...
		.food(food => food.nutrition(0).saturation(0) // [PORT] hunger() -> nutrition()
	...
	event.create('tfg:food/ice_soup')
		...
		.food(food => {
			// [PORT-CHECK] эффект 'tfg:cooling' — кастомный mob effect TFG, ещё не портирован;
			// вернуть строку, когда эффекты TFG будут зарегистрированы.
	...
	event.create('tfg:food/raw_birt')
		...
		.food(food => food.nutrition(1).saturation(1) // [PORT] hunger() -> nutrition()
			// [PORT] species отсутствует — эффект 'species:birtd' и playsound убраны:
