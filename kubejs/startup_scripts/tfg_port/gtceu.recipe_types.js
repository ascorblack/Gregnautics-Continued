...
const registerGTCEuRecipeTypes = (event) => {
	...
	// [PORT] Глобальный биндинг FillDirection удалён из GregTechKubeJSPlugin в GTM 8;
	// [PORT] класс остался (поставляется через jarjar ldlib-neoforge-1.0.35 внутри gtceu.jar) — грузим напрямую.
	...
	event.create('large_solar_panel')
		...
		.setProgressBar(GuiTextures.PROGRESS_BAR_ARROW, $FillDirection.LEFT_TO_RIGHT) // [PORT] FillDirection -> $FillDirection (см. выше)
	...
	event.create('large_solar_panel_tier2')
		...
		.setEUIO($EnergyIO.OUT)
		.setMaxIOSize(2, 0, 2, 1)
		.setProgressBar(GuiTextures.PROGRESS_BAR_ARROW, $FillDirection.LEFT_TO_RIGHT) // [PORT]
		.setSound(GTSoundEntries.COOLING)
	...
	*/
	event.create('ostrum_harvester')
		...
		.setProgressBar(GuiTextures.PROGRESS_BAR_GAS_COLLECTOR, $FillDirection.LEFT_TO_RIGHT) // [PORT]
		.setSound(GTSoundEntries.TURBINE)
	...
	event.create('coal_liquefaction_tower')
		...
		.setProgressBar(GuiTextures.PROGRESS_BAR_ARC_FURNACE, $FillDirection.LEFT_TO_RIGHT) // [PORT]
