...
const registerGTCEuTagPrefixes = (event) => {
	...
	const $Registries = Java.loadClass('net.minecraft.core.registries.Registries') // [PORT] ForgeRegistries (Forge) -> ванильные ResourceKey из Registries (NeoForge)
	const $ResourceLocation = Java.loadClass('net.minecraft.resources.ResourceLocation')

	const createBlockTag = (path) =>
		$TagKeyClass.create($Registries.BLOCK, $ResourceLocation.parse(path)) // [PORT] BLOCKS.getRegistryKey() -> Registries.BLOCK; new ResourceLocation(...) -> ResourceLocation.parse(...)

	/* Tag Fixes */
	// [PORT] GTM 8 по умолчанию использует plates/%s и double_plates/%s (проверено по TagPrefix.class),
	// [PORT] принимаем дефолт GTM 8 (решение roadmap: sheets/ vs plates/) — переопределения убраны.
	// [PORT] TagPrefix.plate.defaultTagPath("sheets/%s")
	// [PORT] TagPrefix.plateDouble.defaultTagPath("double_sheets/%s")
