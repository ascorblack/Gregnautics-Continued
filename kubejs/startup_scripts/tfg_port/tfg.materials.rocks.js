...
const registerTFGRockMaterials = (event) => {
    ...
    const registerStoneMaterial = (rockType, color, components, fluid, temp) => {
        ...
        if (fluid === true) {
			...
			// [PORT-WORKAROUND] GTM8-SNAPSHOT: материал с предметными свойствами без жидкости оставляет незарегистрированный intrusive holder (краш заморозки реестров); molten-жидкость канонична для GT. Убрать при фиксе апстрима.
			// [PORT-FIX] Перенесён из безусловной цепочки в else: для fluid===true (igneous_mafic) свой .liquid(fluidBuilder) уже добавляет жидкость -> двойной gtceu:liquid ("already queued").
			builder.liquid();
		}
    };
