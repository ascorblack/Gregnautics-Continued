...
ServerEvents.tags('item', event => {
	console.info('[Gregnautics] progress: tfg_port rnr item tags start')
	...
	// [PORT] Тэги tfg:* раньше поставлялись модом TFG (data/tfg/tags), которого нет в 1.21.1.
	// Регистрируем их здесь, т.к. они нужны рецептам бетонных дорог в rnr.recipes.js.
	// [PORT-CHECK] состав тэгов восстановлен по смыслу рецептов — сверить в игре с оригиналом.
	...
	event.add('tfg:stone_brick', 'minecraft:stone_bricks')
	global.TFC_STONE_TYPES.forEach(stone => {
		event.add('tfg:stone_brick', `tfc:rock/bricks/${stone}`)
	})

});

ServerEvents.tags('block', event => {
	console.info('[Gregnautics] progress: tfg_port rnr block tags start')
