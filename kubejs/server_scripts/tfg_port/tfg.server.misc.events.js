...
// [PORT] Порт tfg/events.js из TerraFirmaGreg-Modern (1.20.1) на KubeJS 7 / NeoForge 1.21.1.
// [PORT] Обработчики BlockEvents были зарегистрированы напрямую и в оригинале — структура сохранена.
// [PORT] Лог вынесен на уровень файла — rightClicked-обработчики срабатывают на каждый клик.

console.info('[Gregnautics] progress: tfg_port tfg.server.misc events start')

// [PORT-Ф2] TFGServerEvents.dimensionalPowerConsumption — Java-биндинг TFG-Core, отсутствует в 1.21-инстансе.
// [PORT-Ф10] Измерения ad_astra — космоконтент (stellaris не авто-ремапим).
...
// Vase Sounds
// [PORT] Вазы tfg:decorative_vase/* зарегистрированы (startup_scripts/tfg_port/tfg.blocks.vases.js)
...
// [PORT] tfg:armor_stand_arms зарегистрирован (startup_scripts/tfg_port/tfg.items.primitive.js)

BlockEvents.rightClicked(event => {
	let item = event.item
	if (item.id !== 'tfg:armor_stand_arms') return
	let mob = event.block.offset(event.facing).createEntity('minecraft:armor_stand') // [PORT-FIX] block[event.facing] -> block.offset(event.facing) (KubeJS 7)
...
// [PORT-Ф2] su.terrafirmagreg.core.config.TFGConfig — класс TFG-Core, мода нет в 1.21-инстансе
// (Java.loadClass упал бы при загрузке файла). Ограничения на майнеры не портированы.
// [PORT-Ф10] ad_astra:venus / ad_astra:mercury — космоконтент.
...
// [PORT-Ф4-TODO] блок tfg:spider_sac не зарегистрирован в 1.21-порте;
// [PORT] endermanoverhaul (endermanoverhaul:scarab) отсутствует — обработчик отключён.
