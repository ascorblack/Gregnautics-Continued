...
// [PORT] KubeJS 7: изолированный scope — регистрируем событие напрямую вместо registerMoreRedRecipes(event)
ServerEvents.recipes(event => {
	console.info('[Gregnautics] progress: tfg_port morered recipes start')
	...
	event.shaped('2x morered:redwire_post', ['A', 'B'], {
		...
		B: '#c:fine_wires/red_alloy' // [PORT] forge -> c
	...
	event.recipes.gtceu.assembler('tfg/morered/bundled_cable_post_plate')
		.itemInputs('morered:bundled_cable_post', '3x morered:bundled_network_cable', '#c:plates/wrought_iron') // [PORT] forge -> c
		.circuit(6)
	...
	event.shapeless('12x morered:red_alloy_wire', [
		'#c:fine_wires/red_alloy', // [PORT] forge -> c
		'#c:fine_wires/red_alloy', // [PORT] forge -> c
		'#c:fine_wires/red_alloy' // [PORT] forge -> c
	...
	event.recipes.gtceu.chemical_bath(`tfg:morered/wire_decolor`)
		...
		.duration(300)
		.EUt(4)
		.category(GTRecipeCategories.CHEM_DYES) // [PORT-CHECK] категория CHEM_DYES — проверить наличие в GTM 8 (может быть TFG-специфичной)
