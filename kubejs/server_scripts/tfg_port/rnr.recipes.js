...
ServerEvents.recipes(event => {
	console.info('[Gregnautics] progress: tfg_port rnr recipes start')
	...
	event.recipes.gtceu.assembler('rnr:assembler/crushed_base_course')
		.itemInputs('1x #c:gravel', '1x #rnr:loose_rock_items') // [PORT] forge: -> c:
	...
	global.TFC_STONE_TYPES.forEach(stone => {
		...
		event.recipes.gtceu.forming_press(`tfg:macadam_road/${stone}`)
			...
			.notConsumableItem('gtceu:empty_mold') // [PORT-FIX] notConsumable(string) не существует в GTCEu 1.21 — notConsumableItem
	...
	const WET_CONCRETES = [
		...
		{ type: '_control_joint',	mold:'#c:rods'}, // [PORT] forge: -> c:
		{ type: '_panel',			mold:'#c:smooth_stone'}, // [PORT] forge: -> c: // [PORT-CHECK] тэг c:smooth_stone может отсутствовать в NeoForge-конвенциях
		{ type: '_sett',			mold:'#tfg:stone_brick'}, // [PORT] тэг tfg:stone_brick регистрируется в rnr.tags.js (мод TFG отсутствует)
	...
	WET_CONCRETES.forEach(concrete => {
		event.recipes.gtceu.forming_press(`tfg:wet_concrete_road${concrete.type}`)
			.itemInputs('#tfg:wet_concrete_roads') // [PORT] тэг tfg:wet_concrete_roads регистрируется в rnr.tags.js (мод TFG отсутствует)
			.notConsumableItem(`${concrete.mold}`) // [PORT-FIX] notConsumable(string) не существует в GTCEu 1.21 — notConsumableItem
	...
	BAD_CONCRETE.forEach(type => {
		event.custom({
			type: 'rnr:mattock',
			ingredient: [`rnr:${type}_concrete_road`], // [PORT-FIX] BlockIngredient: строка без # трактуется как тег — нужен массив
			result: 'rnr:base_course',
			mode: `tfc:smooth`}) // [PORT-FIX] chisel_mode требует неймспейс
			.id(`tfg:rnr/mattock/recycle_${type}_concrete_road`)

		event.custom({
			type: 'rnr:mattock',
			ingredient: [`rnr:${type}_concrete_road_stairs`], // [PORT-FIX] массив + неймспейс
			result: 'rnr:base_course',
			mode: 'tfc:smooth'})
		...
		event.custom({
			type: 'rnr:mattock',
			ingredient: [`rnr:${type}_concrete_road_slab`], // [PORT-FIX] массив + неймспейс
			result: 'rnr:base_course',
			mode: 'tfc:smooth'})
	...
	event.recipes.gtceu.mixer('rnr:mixer/wet_concrete_mix')
		.inputFluids('gtceu:concrete 200', "#tfg:clean_water 800") // [PORT-CHECK] тэг флюидов tfg:clean_water нигде не регистрируется в 1.21.1 (тот же вопрос в ae2.recipes.js) — нужна регистрация тэга или замена на воду
	...
	// [PORT-Ф4] tfg: почвы (tfg:dirt/*, tfg:mud/*, tfg:tamped/*, tfg:coarse_dirt/*, tfg:duff/*) появятся в фазе 4 — весь блок закомментирован
	/*
	...
	*/
	...
	// [PORT-Ф4] tfg:asphalt_road* появятся в фазе 4 — рецепты закомментированы
	/*
	event.custom({
		type: 'rnr:mattock',
