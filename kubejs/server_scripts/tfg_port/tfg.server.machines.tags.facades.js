...
// [PORT] Порт tfg/machines/tags.facades.js из TerraFirmaGreg-Modern (1.20.1) на KubeJS 7 / NeoForge 1.21.1.
// [PORT] Белый список блоков для фасадов GT (тег tfg:whitelisted/facades). Сами рецепты фасадов пока
//   закомментированы в gregtech.recipes.js (NBT->data components, [PORT-CHECK]) — тег наполняем заранее.
// [PORT] Вырезаны отсутствующие моды: domum_ornamentum, simplylight, everycomp, betterend.
// [PORT-Ф10] Теги #tfg:ad_astra_* — космоконтент (stellaris не авто-ремапим), закомментированы.
// [PORT-FIX] forge:->c: (c:glass_blocks, c:cobblestones, c:sandstone/blocks [PORT-CHECK], c:concretes);
//   исправлена опечатка оригинала `gtceu:chiseled${m}` -> `gtceu:chiseled_${m}` (проверено по реестру).

ServerEvents.tags('item', event => {
	console.info('[Gregnautics] progress: tfg_port tfg.server.machines tags.facades start')
	...
	const facade_materials = [
		...
		'#tfc:rock/raw',
		'#tfc:metal_plated_blocks', // [PORT-CHECK] тег должен наполняться другим tfg-скриптом
		'#tfc:dirt',
		'#tfc:grass',
		...
		'#tfc:mud_bricks',
		'#c:sandstone/blocks', // [PORT-CHECK] forge:sandstone -> c:sandstone/blocks (конвенция NeoForge 1.21)
		'#c:storage_blocks',
		'#c:glass_blocks', // [PORT] forge:glass -> c:glass_blocks
		'#minecraft:terracotta',
		'#c:concretes',
		'#create:casing',
		// '#domum_ornamentum:brick_items', // [PORT] domum_ornamentum отсутствует в сборке 1.21.1
		// '#simplylight:any_lamp_on', // [PORT] simplylight отсутствует
		// '#simplylight:any_lamp_off', // [PORT] simplylight отсутствует
		'#minecraft:planks',
		'#minecraft:logs',
		'#c:cobblestones', // [PORT] forge:cobblestone -> c:cobblestones
		'#railways:palettes/cycle_groups/base',

		// '#tfg:ad_astra_iron_blocks', // [PORT-Ф10]
		// '#tfg:ad_astra_steel_blocks', // [PORT-Ф10]
		// '#tfg:ad_astra_desh_blocks', // [PORT-Ф10]
		// '#tfg:ad_astra_ostrum_blocks', // [PORT-Ф10]
		// '#tfg:ad_astra_calorite_blocks', // [PORT-Ф10]
		// '#tfg:ad_astra_etrium_blocks', // [PORT-Ф10]
		// '#tfg:titanium_concrete', // [PORT-Ф4-TODO] тег пока нигде не наполняется
		...
		// 'everycomp:c/domum_ornamentum/cactus_window', // [PORT] everycomp отсутствует
		// 'everycomp:c/domum_ornamentum/cactus_extra_window', // [PORT] everycomp отсутствует
		'ae2:quartz_glass',
		...
		// better end
		// 'betterend:brimstone', // [PORT] betterend отсутствует
		// 'betterend:lacugrove_bark', // [PORT] betterend отсутствует
		// 'betterend:lucernia_bark', // [PORT] betterend отсутствует
	];
	...
	gtceu_stonelike_fm.forEach(facade_material => {
		...
		event.add('tfg:whitelisted/facades', `gtceu:chiseled_${  facade_material}`) // [PORT-FIX] было `chiseled${m}` (опечатка оригинала)
	...
	//everycomp windows
	// [PORT] everycomp отсутствует в сборке 1.21.1
