...
// [PORT] Из TFG server_scripts/tfg/primitive/recipes.cloth.js (Ф4).
// Главные изменения 1.20→1.21:
//  - forge:→c: (rods/wooden)
//  - damage_inputs_shapeless_crafting удалён в kubejs_tfc 2.0 — обычный shapeless (TFC 1.21 сам даёт remainder)
//  - casting: конструктор (result, mold, fluid) + .breakChance(); TFC.fluidStackIngredient удалён
//  - anvil: .bonus(true) → .applyBonus()
//  - vintageimprovements → createvintageneoforged; global.CLOTH_VI_DURATION_MULT
//    в 1.21 не задаётся глобально — локальная константа (= 4, как в createvintageneoforged.recipes.js)
//  - .springColor() удалён — схемы coiling CVI 1.21.1 не имеют такого поля

const CLOTH_VI_DURATION_MULT = 4; // [PORT]

ServerEvents.recipes(event => {
	console.info('[Gregnautics] progress: tfg_port tfg.server.primitive.recipes.cloth start')
	...
	event.recipes.gtceu.macerator('macerate_wool')
		...
		.chancedOutput('tfc:wool_yarn', 9000)
		.chancedOutput('tfc:wool_yarn', 5000)
		.chancedOutput('tfc:wool_yarn', 2000)
		.duration(200)
		.EUt(2)

	// Jute Fiber
	global.generateMixerRecipe(event, 'tfc:jute', "#tfg:clean_water 200",
	...
	spindle_metals.forEach(spindle => {
		...
		).applyBonus().tier(spindle.tier).id(`tfg:anvil/${spindle.metal}_spindle_head`) // [PORT-FIX] .bonus(true) -> .applyBonus()
	})

	spindle_metals_cast.forEach(spindle => {
		// [PORT-FIX] kubejs_tfc 2.0: casting(result, mold, fluid) + .breakChance(); TFC.fluidStackIngredient удалён
		event.recipes.tfc.casting(
			`tfg:${spindle.metal}_spindle_head`,
			'tfg:spindle_head_mold',
			Fluid.of(`gtceu:${spindle.metal}`, 72)
		).breakChance(0.1).id(`tfg:casting/${spindle.metal}_spindle_head`)
	})
	...
	// [PORT-FIX] kubejs_tfc 2.0: scraping(result, ingredient, output_texture, input_texture) + .extraDrop()
	event.recipes.tfc.scraping(
		'tfg:flax_waste',
		'tfg:flax_product',
		'tfg:item/flax_waste',
		'tfg:item/flax_product'
	).extraDrop('2x tfg:flax_line').id('tfg:scraping/flax_line')
	...
	).extraDrop('18x tfg:flax_line').id('tfg:scraping/flax_line_from_bundle')
	...
	event.recipes.tfc.scraping(
		...
		'tfg:item/flax_waste'
	).extraDrop('tfg:flax_tow').id('tfg:scraping/flax_tow')
	...
	event.recipes.tfc.scraping(
		...
		'tfc:item/groundcover/humus',
		'tfg:item/bundled_scraped_flax'
	).extraDrop('9x tfg:flax_tow').id('tfg:scraping/flax_tow_from_pile')
	...
	// [PORT-FIX] .minimalRPM() удалён: CentrifugationRecipe в CVI 1.21.1 — обычный ProcessingRecipe без поля RPM (проверено по jar)
	event.recipes.createvintageneoforged.centrifugation(['2x tfg:flax_line', 'tfg:flax_tow', 'tfc:groundcover/humus'], 'tfg:flax_product') // [PORT] vintageimprovements -> createvintageneoforged
		.processingTime(200 * CLOTH_VI_DURATION_MULT)
		.id('tfg:vi_seperate_flax')

	event.recipes.createvintageneoforged.centrifugation(['18x tfg:flax_line', '9x tfg:flax_tow', '9x tfc:groundcover/humus'], 'tfg:flax_bundle') // [PORT]
		.processingTime(1500 * CLOTH_VI_DURATION_MULT)
	...
	// [PORT-FIX] .springColor() удалён — схемы coiling CVI 1.21.1 не имеют такого поля
	event.recipes.createvintageneoforged.coiling('4x tfg:linen_thread', 'tfg:flax_line') // [PORT]
		.processingTime(20 * CLOTH_VI_DURATION_MULT)
