...
// [PORT] Из TFG server_scripts/tfg/ores_and_materials/recipes.mo_si_b.js (registerTFGMoSiBRecipes).
// [PORT-Ф2] ФАЙЛ ПОЛНОСТЬЮ ЗАБЛОКИРОВАН:
//   - Java-классы fi.dea.mc.deafission.* (мод deafission) отсутствуют в 1.21.1-сборке;
//   - все tfg:*mo_si_b* предметы/жидкости — кастомные GT-материалы TFG (Фаза 2);
//   - типы рецептов gas_pressurizer / ostrum_linear_accelerator — кастомные (Ф2/Ф7);
//   - dimension 'ad_astra:mars' -> stellaris (Фаза 10, [PORT-Ф10] не авто-ремапить).
// Удаления стоковых gtceu:*mo_si_b* рецептов не применены: в GTM8 материала mo_si_b нет
//   (он тоже кастомный TFG), удалять нечего.

console.info('[Gregnautics] progress: tfg_port tfg.server.ores.recipes.mo_si_b [PORT-Ф2] пропущен (см. шапку)')
