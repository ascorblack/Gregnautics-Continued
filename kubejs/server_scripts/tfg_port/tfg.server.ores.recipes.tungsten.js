...
// [PORT] Из TFG server_scripts/tfg/ores_and_materials/recipes.tungsten.js (registerTFGTungstenRecipes).
// [PORT-Ф2] ФАЙЛ ПОЛНОСТЬЮ ЗАБЛОКИРОВАН Фазой 2: tfg:sodium_tungstate_dust, tfg:ammonium_tungstate_dust,
//   tfg:apt_gem, tfg:tungsten_oxide_dust — кастомные GT-материалы TFG (tfg.materials.tungsten.js
//   определён, но диспетчер Ф2 отключён); тип рецепта evaporation_tower — кастомный (Ф7).
// ВАЖНО: удаления стоковой вольфрамовой цепочки GT (tungstic_acid_* / tungstic_acid_electrolysis)
//   СОЗНАТЕЛЬНО НЕ ПРИМЕНЕНЫ — иначе пак остался бы без пути к вольфраму. Применить вместе с Ф2.

console.info('[Gregnautics] progress: tfg_port tfg.server.ores.recipes.tungsten [PORT-Ф2] пропущен (см. шапку)')
