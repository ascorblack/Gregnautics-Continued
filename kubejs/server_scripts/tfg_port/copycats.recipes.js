...
// [PORT] KubeJS 7: регистрация напрямую через ServerEvents.recipes вместо диспетчера registerCreateCopycatsRecipes
ServerEvents.recipes(event => {
	console.info('[Gregnautics] progress: tfg_port copycats recipes start')
	...
	global.TFGDamageShaped(event,'copycats:copycat_fluid_pipe', [
		...
		B: '#c:plates/zinc' // [PORT] forge:plates/zinc -> c:plates/zinc
	...
	COPYCAT_DATA.forEach(copycat => {
		// Stonecutter Recipes
		event.stonecutting(`${copycat.multiplier}x copycats:${copycat.id}`, "#c:ingots/zinc") // [PORT] forge:ingots/zinc -> c:ingots/zinc
	...
	event.recipes.gtceu.assembler('tfg:copycats/copycat_fluid_pipe')
		.itemInputs('2x #c:plates/zinc') // [PORT] forge:plates/zinc -> c:plates/zinc
		.circuit(3)
