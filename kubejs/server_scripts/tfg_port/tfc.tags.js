...
// [PORT] Из TFG server_scripts/tfc/tags.js (Фаза 3).
// Главные изменения 1.20→1.21:
//  - forge:→c: (исключения из remap_tables.json: tools/knives→tools/knife и т.п.)
//  - ForgeRegistries → BuiltInRegistries, forge FluidStack → neoforge FluidStack
//  - отсутствующие моды вырезаны с пометкой [PORT], tfg:-контент Ф4 закомментирован [PORT-Ф4]
// Ключевая часть Ф3: униф. форм слитков TFC↔GT + скрытие TFC-жидкостей металлов
// (закрывает дубль Copper Ingot в JEI).

const $TfcBuiltInRegistries = Java.loadClass('net.minecraft.core.registries.BuiltInRegistries');
const $TfcResourceLocation = Java.loadClass('net.minecraft.resources.ResourceLocation');
// [PORT] Java.loadClass + const внутри функции-обработчика даёт Rhino-ошибку
// "redeclaration of var" — все loadClass только на верхнем уровне файла.
...
const $NeoFluidStack = Java.loadClass('net.neoforged.neoforge.fluids.FluidStack');

/** Тихая проверка существования предмета — не создаёт warning'ов в логе. */
function tfcItemExists(id) {
	try {
		return $TfcBuiltInRegistries.ITEM.containsKey($TfcResourceLocation.parse(id));
	} catch (e) {
		return false;
	}
}
...
ServerEvents.tags('item', event => {
	console.info('[Gregnautics] progress: tfg_port tfc item tags start');
	...
	try {
		...
		console.warn(`[Gregnautics][PORT-CHECK] GT->TFC tool tags failed: ${e}`);
	}

	// Add chainsaws to #tfc:axes so they drop fruit tree saplings
	event.add("tfc:axes", "#c:tools/chainsaws");

	/**
	 * @type {string[]} - Item IDs and tags usable on tfc tool racks.
	 * [PORT] вырезаны: buildinggadgets2, measurements, chalk, computercraft (Ф10),
	 * grapplemod, mcw_tfc_aio, tfclunchbox, primitive_creatures, species — модов нет в 1.21.1.
	 */
	const usableOnToolRack = [
		"#c:tools/mining_hammer",
		"#c:tools/spade",
		"#c:tools/wrench",
		"#c:tools/file",
		"#c:tools/crowbar",
	...
	usableOnToolRack.forEach((entry) => {
		if (entry.startsWith('#') || tfcItemExists(entry)) {
			event.add("tfc:usable_on_tool_rack", entry);
		}
	});

	//liquid non placeable
	// [PORT] вырезаны tfc_gourmet (мода нет); tfcagedalcohol/firmalife есть
	...
	// Для складывания
	event.add("tfc:pileable_ingots", "#c:ingots");
	event.add("tfc:pileable_sheets", "#c:plates");
	...
	event.remove("c:dusts/sulfur", "tfc:powder/sulfur");
	...
	// [PORT] beneath:cursecoal — мода нет в 1.21.1
	...
	// Glassworking sands
	// [PORT] ad_astra пески — мода нет в 1.21.1
	...
	//Cloth
	event.add("c:cloth", "tfc:burlap_cloth");
	event.add("c:cloth", "tfc:wool_cloth");
	event.add("c:cloth", "tfc:silk_cloth");
	...
	// [PORT] tfcchannelcasting / greate forge_invisible_whitelist — модов нет в 1.21.1

	event.add("c:mushrooms", "tfc:plant/artists_conk");
	...
	// [PORT-FIX Ф4] tfg:mud_bricks/* зарегистрированы в startup_scripts/tfg_port/tfg.blocks.mud_bricks.js;
	// tfg:mud_brick/* (сырые) и tfg:drying_bricks/* нигде не регистрируются startup-скриптами — оставлены закомментированными
	global.TFG_MUD_TYPES.forEach(mud => {
		// [PORT-Ф4] event.add("tfc:wet_mud_bricks", `tfg:mud_brick/${mud}`);
		event.add("tfc:mud_bricks", `tfg:mud_bricks/${mud}`); // [PORT-FIX Ф4]
		// [PORT-Ф4] event.add("tfc:dry_mud_bricks", `tfg:drying_bricks/${mud}`);
	})
	...
	event.add('c:tools/knife', 'tfc:stone/knife/sedimentary')
	event.add('c:tools/knife', 'tfc:stone/knife/metamorphic')
	event.add('c:tools/knife', 'tfc:stone/knife/igneous_intrusive')
	event.add('c:tools/knife', 'tfc:stone/knife/igneous_extrusive')

	//greens and browns
	// [PORT-FIX] TFC 1.21: compost_browns_low/greens_low -> compost_browns/low, compost_greens/low
	event.remove('tfc:compost_browns/low', '#tfc:fallen_leaves')
	event.add('tfc:compost_greens/low', '#tfc:fallen_leaves')
	event.add('tfc:compost_greens/low', '#minecraft:leaves')

	// Powder dyes
	event.add('c:dyes/blue', 'tfc:powder/lapis_lazuli')
	event.add('c:dyes/blue', 'tfc:powder/graphite')
	event.add('c:dyes/black', 'tfc:powder/charcoal')
	event.add('c:dyes/black', 'tfc:powder/coke')
	event.add('c:dyes/pink', 'tfc:powder/kaolinite')
	event.add('c:dyes/orange', 'tfc:powder/sylvite')
	event.add('c:dyes/orange', 'tfc:powder/copper')
	event.add('c:dyes/light_gray', 'tfc:powder/silver')
	event.add('c:dyes/gray', 'tfc:powder/cassiterite')
	event.add('c:dyes/gray', 'tfc:powder/magnetite')
	event.add('c:dyes/gray', 'tfc:powder/sphalerite')
	event.add('c:dyes/gray', 'tfc:powder/tetrahedrite')
	event.add('c:dyes/green', 'tfc:powder/bismuthinite')
	event.add('c:dyes/green', 'tfc:powder/malachite')
	event.add('c:dyes/brown', 'tfc:powder/garnierite')
	event.add('c:dyes/yellow', 'tfc:powder/limonite')
	event.add('c:dyes/yellow', 'tfc:powder/native_gold')

	//Hanging Signs Soft & Hard Wood
	// [PORT] beneath вырезан — мода нет в 1.21.1; afc есть
	...
	// [PORT-Ф4] event.add('tfc:fluxstone', 'tfg:loose/dripstone')
	...
	event.add('c:tools/igniter', 'tfc:firestarter');
...
ServerEvents.tags('block', event => {
	console.info('[Gregnautics] progress: tfg_port tfc block tags start');
	...
	// [PORT] ForgeRegistries.BLOCKS → BuiltInRegistries.BLOCK (итерируем по ключам)
	const blacklist = ["drying", "slab", "stairs", "wall", "additionalplacements", "fence", "roof", "bridge"];
	const matches = [];
	$TfcBuiltInRegistries.BLOCK.keySet().forEach(key => {
		const id = String(key);
	...
	// [PORT-Ф4] event.add('tfc:mineable_with_glass_saw', 'tfg:casings/machine_casing_bioculture_glass');
	// [PORT] expatternprovider → extendedae (ремап modid)
	event.add('tfc:mineable_with_glass_saw', 'extendedae:assembler_matrix_glass');
...
ServerEvents.tags('fluid', event => {
	console.info('[Gregnautics] progress: tfg_port tfc fluid tags start');
	...
	// [PORT-Ф2] gtceu:aluminium_silicate — материал TFG (Ф2)
	...
	***********************************************************************************************************/
	...
	// [PORT] ForgeRegistries.FLUIDS → BuiltInRegistries.FLUID; forge FluidStack → neoforge FluidStack
	// ($FluidAttribute / $NeoFluidStack загружены на верхнем уровне файла)
	...
	$TfcBuiltInRegistries.FLUID.forEach(fluid => {
		...
		const hotFluids = (fluid) => {
			...
			try {
				...
				try {
					temp = fluid.getFluidType().getTemperature(new $NeoFluidStack(fluid, 1000));
				} catch (e2) {
					temp = -1;
				}
			}

			const id = $TfcBuiltInRegistries.FLUID.getKey(fluid);
		...
		const liquidTag = (fluid) => {
			...
			try { gaseousWrapper = event.get('c:gaseous'); } catch (e) { /* ignore */ }
			try { plasmaticWrapper = event.get('c:plasmatic'); } catch (e) { /* ignore */ }
			try { hiddenWrapper = event.get('c:hidden_from_recipe_viewers'); } catch (e) { /* ignore */ }
		...
		const neutralTag = (fluid) => {
			...
			try { acidicWrapper = event.get('c:acidic'); } catch (e) { /* ignore */ }
			...
			event.add('c:neutral_fluids', idString);
		};
	...
	// Can use any liquid.
	event.add('tfc:usable_in_pot', '#c:neutral_fluids')
	event.add('firmalife:usable_in_vat', '#c:neutral_fluids')

	// Can use only neutral fluids.
	event.add('tfc:usable_in_barrel', '#c:neutral_fluids');
	event.add('tfc:usable_in_wooden_bucket', '#c:neutral_fluids');
	// Ceramic jugs and glass bottles
	event.add('tfc:usable_in_jug', '#c:neutral_fluids');
	event.add('firmalife:usable_in_mixing_bowl', '#c:neutral_fluids');

	// All fluids are given the '#tfg:not_solid' tag incase we have a use for them regardless of state.
	event.add('tfg:not_solid', '#c:liquid')
	event.add('tfg:not_solid', '#c:gaseous')
	event.add('tfg:not_solid', '#c:plasmatic')
	...
	// [PORT] tfg:alcohols (tfc_gourmet:nalivka) — мода нет в 1.21.1
