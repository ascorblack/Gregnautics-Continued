...
// [PORT] Из TFG server_scripts/tfg/ores_and_materials/recipes.material_tag_prefixes.js
//   (processDust/processIngot/processPlate/processRod/... — обработка tag prefix'ов материалов).
// [PORT-Ф2] ФАЙЛ ПОЧТИ ПОЛНОСТЬЮ ЗАБЛОКИРОВАН (см. tfg.server.ores.recipes.materials.js):
//   TFGPropertyKey.TFC_PROPERTY / TFGTagPrefix / TFGMaterialFlags / greate / TFGHelpers.
// АКТИВНО ПОРТИРОВАНА только генерация createaddition:rolling (слиток -> пластина -> фольга) —
//   аддитивные рецепты на сток-предметах, см. хэндлер ниже. Интерим-замену формовки даёт
//   gregnautics_create_metal_forming.js.
// [PORT-CHECK] Удаления gtceu:shaped/plate_* и переопределения bender'ов НЕ применены
//   (без TFC-наковальни они оставили бы пак без ручной альтернативы).

ServerEvents.recipes(event => {
	console.info('[Gregnautics] progress: tfg_port tfg.server.ores.recipes.material_tag_prefixes start')

	// [PORT] Активная часть processPlate/processFoil: createaddition:rolling (формат 1.21:
	// result {id,count}). Аддитивно, только для видимых сток-предметов.
	try {
		global.forEachMaterial(material => {
			const materialName = material.getName();

			const plateItem = ChemicalHelper.get(TagPrefix.plate, material, 1)
			if (plateItem === null || plateItem.isEmpty() || plateItem.hasTag('c:hidden_from_recipe_viewers'))
				return;
			...
			if (ingotItem !== null && !ingotItem.isEmpty()) {
				// [PORT-FIX] createaddition:rolling 1.21: ingredients[] + results[] (проверено по data/create/recipe/rolling/bamboo.json в jar)
				event.custom({
					type: "createaddition:rolling",
					ingredients: [{ item: ingotItem.getId() }],
					results: [{ id: plateItem.getId() }]
			...
			if (foilItem !== null && !foilItem.isEmpty() && !plateItem.hasTag('tfg:no_vintage_gen')) {
				// [PORT-FIX] createaddition:rolling 1.21: ingredients[] + results[]
				event.custom({
					type: "createaddition:rolling",
					ingredients: [{ item: plateItem.getId() }],
					results: [{ id: foilItem.getId(), count: 4 }]
				}).id(`tfg:rolling/${materialName}_foil`)
			}
		})
	} catch (e) {
		console.error(`[Gregnautics][PORT-CHECK] tfg.server.ores.recipes.material_tag_prefixes: цикл rolling упал: ${e}`)
	}

	console.info('[Gregnautics] progress: tfg_port tfg.server.ores.recipes.material_tag_prefixes done')
})
