...
// [PORT] Порт из TFG-Modern 1.20.1: gregtech/recipes.machines.js
// [PORT] KubeJS 7: функция registerGTCEuMachineRecipes заменена на прямую регистрацию ServerEvents.recipes
// [PORT] forge:* теги -> c:* (tools/wrenches->tools/wrench, tools/hammers->tools/hammer, tools/saws->tools/saw,
//        forge:glass->c:glass_blocks, buzz_saw_heads->c:buzz_saw_blades)
// [PORT-FIX] gtceu:palladium_substation переименован в GTM 8.0 -> gtceu:palladium_substation_casing
// [PORT-FIX] removeMaceratorRecipe — локальная копия хелпера из gregtech/recipes.removes.js (KubeJS 7: файлы изолированы);
//        строки greate:* удалены — мод greate отсутствует в сборке
// [PORT-Ф4] TFGHelpers.registerMaterialInfo — хелпер мода TFG, отсутствует в сборке (Фаза 4) — закомментирован

/** [PORT-FIX] локальный хелпер (см. шапку) */
function removeMaceratorRecipe(event, id) {
	event.remove({ id: `gtceu:macerator/${id}` })
	// [PORT] greate отсутствует: greate:milling / greate:crushing удаления не нужны
}

ServerEvents.recipes(event => {
	console.info('[Gregnautics] progress: tfg_port gregtech.recipes.machines start')
	...
	// [PORT-Ф2-TEMP] small_gears/wrought_iron не генерируется в GTM 8.0 (TFG-флаг GENERATE_SMALL_GEAR
	// на WroughtIron заблокирован Ф2) — временная замена на small_gears/iron (gtceu:small_iron_gear есть)
	...
	// [PORT-Ф4] TFGHelpers.registerMaterialInfo('gtceu:hp_steam_rock_crusher', [GTMaterials.WroughtIron, 10.5, GTMaterials.Steel, 4, GTMaterials.TinAlloy, 2])
	...
	// [PORT-Ф4] TFGHelpers.registerMaterialInfo('gtceu:hp_steam_miner', [GTMaterials.WroughtIron, 11, GTMaterials.Steel, 5, GTMaterials.TinAlloy, 2])

	//#endregion

	//#region Passthrough hatches
	// [PORT-Ф2-TEMP] gtceu:small_wrought_iron_gear не генерируется в GTM 8.0 — замена на gtceu:small_iron_gear
	...
	event.remove({ id: 'gtceu:shaped/passthrough_hatch_fluid' })
	...
	event.replaceInput({ id: 'gtceu:shaped/ulv_machine_hull' }, '#c:plates/wood', '#c:plates/lead')
	...
	event.recipes.gtceu.assembler('long_distance_item_pipe')
		.itemInputs('1x #c:large_item_pipes/polyvinyl_chloride', '4x #c:plates/stainless_steel')
	...
	event.recipes.gtceu.assembler('long_distance_fluid_pipe')
		.itemInputs('1x #c:large_fluid_pipes/stainless_steel', '4x #c:plates/polyvinyl_chloride')
		.inputFluids(Fluid.of('gtceu:soldering_alloy', 72))
	...
	event.recipes.gtceu.assembler('long_distance_item_endpoint')
		.itemInputs('2x #c:large_item_pipes/polyvinyl_chloride', '8x #c:plates/stainless_steel', '2x #c:gears/stainless_steel')
	...
	event.recipes.gtceu.assembler('long_distance_fluid_endpoint')
		.itemInputs('2x #c:large_fluid_pipes/stainless_steel', '8x #c:plates/polyvinyl_chloride', '2x #c:gears/stainless_steel')
		.inputFluids(Fluid.of('gtceu:soldering_alloy', 144))
	...
	// Drums
	// [PORT-Ф2] gtceu:bismuth_bronze_drum / gtceu:black_bronze_drum и *_crate не зарегистрированы в GTM 8.0 —
	// требуют TFG-модификаций свойств материалов (Фаза 2, upstream bug). Ванильные GT-барабаны
	// (bronze/steel/aluminium и т.д.) остаются доступны, играбельность не страдает.
	...
	// 		.itemInputs(`4x #c:plates/${material}`, `2x #c:rods/long/${material}`)
	...
	// 		.itemInputs(`4x #c:plates/${material}`, `4x #c:rods/long/${material}`)
	...
	event.recipes.gtceu.assembler('tfg:wood_crate')
		.itemInputs('4x #minecraft:planks', '4x #c:screws/wrought_iron')
	...
	event.recipes.gtceu.shaped('gtceu:hv_cutter', [
		...
		A: 'gtceu:gold_single_cable', // [PORT-FIX] тег #c:single_cables/gold пуст в GTM8 (кабели без тегов) -> конкретный предмет
		B: '#gtceu:circuits/hv',
		C: 'gtceu:tempered_glass',
		D: 'gtceu:hv_conveyor_module',
		E: 'gtceu:hv_machine_hull',
		F: 'gtceu:ultimet_buzz_saw_blade', // [PORT-FIX] тег #c:buzz_saw_blades/* не генерируется в GTM8 (нет в TagPrefix) -> конкретный предмет
		G: 'gtceu:hv_electric_motor'
	}).id('gtceu:shaped/hv_cutter')

	// [PORT-Ф4] TFGHelpers.registerMaterialInfo('gtceu:hv_cutter', [GTMaterials.Rubber, 17, GTMaterials.Electrum, 12, GTMaterials.StainlessSteel, 11, GTMaterials.Silver, 6, GTMaterials.Ultimet, 4, GTMaterials.Gold, 2.5])
	...
	// [PORT-FIX] gtceu:palladium_substation -> gtceu:palladium_substation_casing (переименование в GTM 8.0)
	removeMaceratorRecipe(event, 'macerate_palladium_substation')
	event.recipes.gtceu.shaped('gtceu:palladium_substation_casing', [
		'AAA',
		' B ',
		'AAA'
	], {
	...
	event.recipes.gtceu.assembler('casing_palladium_substation')
		...
		.itemOutputs('gtceu:palladium_substation_casing')
		.circuit(6)
		.duration(20 * 2.5)
		.EUt(GTValues.VHA[GTValues.LV])
	...
	event.recipes.gtceu.shaped('gtceu:power_substation', [
		...
		B: 'gtceu:mpic_chip',
		C: '#gtceu:circuits/ev',
		D: 'gtceu:palladium_substation_casing'
	...
	event.recipes.gtceu.assembler('industrial_steam_casing')
		.itemInputs('6x #c:plates/brass', '#c:frames/steel')
	...
	// Steam generators
	// [PORT-Ф2] Замены рецептов паровых турбин (lv/mv/hv) требуют роторов CobaltBrass/Magnalium/VanadiumSteel —
	// GENERATE_ROTOR у этих материалов добавлялся TFG-модификациями (Фаза 2, заблокировано upstream-багом).
	// gtceu:cobalt_brass_rotor / magnalium_rotor / vanadium_steel_rotor не существуют в GTM 8.0.
	// Для играбельности стандартные GT-рецепты турбин НЕ удаляем (event.remove тоже закомментирован).
	...
	// ], {
	// 	A: '#gtceu:circuits/lv',
	// 	B: '#c:huge_fluid_pipes/steel',
	// 	C: '#c:rotors/cobalt_brass',
	// 	D: 'gtceu:lv_machine_hull',
	// 	E: 'gtceu:lv_electric_motor',
	// 	F: '#c:single_cables/tin'
	...
	// ], {
	// 	A: '#gtceu:circuits/mv',
	// 	B: '#c:huge_fluid_pipes/aluminium',
	// 	C: '#c:rotors/magnalium',
	// 	D: 'gtceu:mv_machine_hull',
	// 	E: 'gtceu:mv_electric_motor',
	// 	F: '#c:single_cables/copper'
	...
	// ], {
	// 	A: '#gtceu:circuits/hv',
	// 	B: '#c:huge_fluid_pipes/stainless_steel',
	// 	C: '#c:rotors/vanadium_steel',
	// 	D: 'gtceu:hv_machine_hull',
	// 	E: 'gtceu:hv_electric_motor',
	// 	F: '#c:single_cables/gold'
	// }).addMaterialInfo().id('tfg:shaped/steam_turbine_hv')

	console.info('[Gregnautics] progress: tfg_port gregtech.recipes.machines done')
})
