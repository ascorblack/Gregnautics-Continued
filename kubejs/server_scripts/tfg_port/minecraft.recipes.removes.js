...
// [PORT] Порт minecraft/recipes.removes.js из TerraFirmaGreg-Modern (1.20.1) на KubeJS 7 / NeoForge 1.21.1.
// [PORT] removeMinecraftRecipes(event) -> прямой обработчик ServerEvents.recipes (KubeJS 7: изолированные скоупы,
// диспетчер не используется; из minecraft.recipes.js вызов удалён — оба обработчика выполняются независимо).
// [PORT-CHECK] Часть удаляемых gtceu:-id могла измениться/исчезнуть в GTM 8.0 — отсутствующий id даёт только warning в логе.

ServerEvents.recipes(event => {
	console.info('[Gregnautics] progress: tfg_port minecraft recipes.removes start')
