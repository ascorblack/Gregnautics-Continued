...
// [PORT] Источник: server_scripts/gregtech/recipes.removes.js (TFG 1.20.1)
// [PORT] Регистрация напрямую через ServerEvents.recipes (без диспетчера)
// [PORT] Удаления gt-рецептов, которых может не быть в GTM8 — безвредные предупреждения, порт 1:1
// [PORT] greate отсутствует в сборке — строки с greate:-id закомментированы
// [PORT-FIX] Теги forge: -> c: (forge:chests/wooden -> c:chests/wooden)
// [PORT-FIX] GraniteRed переименован в RedGranite в GTM8 — добавлен дублирующий id decomposition_electrolyzing_red_granite

// [PORT] локальные копии хелперов (KubeJS 7: файлы изолированы)
...
function removeCutterRecipe(event, id) {
	...
	// event.remove({ id: `greate:cutting/integration/gtceu/cutter/${id}` }) // [PORT] greate отсутствует
}

ServerEvents.recipes(event => {
	console.info('[Gregnautics] progress: tfg_port gregtech.recipes.recycling.removes start')

	// [PORT-FIX] {input:...} без типа сканирует ВСЕ рецепты и падает на чтении GT-жидкостных входов
	// [PORT-FIX] (KubeJS читает amount=10000 как ингредиент) -> тип-фильтр только по item-крафту.
	var REPLACE_CRAFTING_FILTERS = [
	...
	event.replaceInput(REPLACE_CRAFTING_FILTERS, 'gtceu:wood_drum', 'minecraft:glass')
	event.replaceInput(REPLACE_CRAFTING_FILTERS, 'minecraft:chest', '#c:chests/wooden') // [PORT-FIX] forge: -> c:
	...
	// event.remove({ id: 'greate:mixing/integration/gtceu/mixer/mossy_cobblestone_from_moss_block' }) // [PORT] greate отсутствует
	// event.remove({ id: 'greate:mixing/integration/gtceu/mixer/mossy_cobblestone_from_vine' }) // [PORT] greate отсутствует
	...
	// event.remove({ id: 'greate:mixing/integration/gtceu/mixer/ruby_slurry_from_crushed_ruby' }) // [PORT] greate отсутствует
	event.remove({ id: 'gtceu:mixer/ruby_slurry_from_washed_ruby' })
	// event.remove({ id: 'greate:mixing/integration/gtceu/mixer/ruby_slurry_from_washed_ruby' }) // [PORT] greate отсутствует

	event.remove({ id: 'gtceu:mixer/sapphire_slurry_from_washed_sapphire' })
	// event.remove({ id: 'greate:mixing/integration/gtceu/mixer/sapphire_slurry_from_washed_sapphire' }) // [PORT] greate отсутствует
	event.remove({ id: 'gtceu:mixer/sapphire_slurry_from_crushed_sapphire' })
	// event.remove({ id: 'greate:mixing/integration/gtceu/mixer/sapphire_slurry_from_crushed_sapphire' }) // [PORT] greate отсутствует

	event.remove({ id: 'gtceu:mixer/green_sapphire_slurry_from_crushed_green_sapphire' })
	// event.remove({ id: 'greate:mixing/integration/gtceu/mixer/green_sapphire_slurry_from_crushed_green_sapphire' }) // [PORT] greate отсутствует
	event.remove({ id: 'gtceu:mixer/green_sapphire_slurry_from_washed_green_sapphire' })
	// event.remove({ id: 'greate:mixing/integration/gtceu/mixer/green_sapphire_slurry_from_washed_green_sapphire' }) // [PORT] greate отсутствует
	...
	event.remove({ id: 'gtceu:electrolyzer/decomposition_electrolyzing_red_granite' }) // [PORT-FIX] GTM8: GraniteRed -> RedGranite
	...
	// event.remove({ id: 'greate:mixing/integration/gtceu/mixer/mossy_red_granite_cobblestone_from_moss_block' }) // [PORT] greate отсутствует
	// event.remove({ id: 'greate:mixing/integration/gtceu/mixer/mossy_red_granite_cobblestone_from_vine' }) // [PORT] greate отсутствует
