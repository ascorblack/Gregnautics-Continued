...
// [PORT] Портировано из mega_cells/recipes.js (TFG-Modern 1.20.1 -> 1.21.1 NeoForge).
// [PORT] Диспетчер registerMegaCellsRecipes(event) заменён на прямую регистрацию ServerEvents.recipes.
// [PORT] Теги forge:* -> c:*; ad_astra отсутствует в сборке 1.21.1 (лунные варианты рецептов удалены,
// [PORT] у безальтернативных рецептов удалена только строка .dimension).

ServerEvents.recipes(event => {
	console.info('[Gregnautics] progress: tfg_port megacells recipes start')
	...
	event.recipes.gtceu.forming_press('megacells:printed_accumulation_processor')
		.itemInputs('#c:dense_plates/silicon') // [PORT] forge: -> c:
		.notConsumableItem('megacells:accumulation_processor_press') // [PORT-FIX] notConsumable(string) не существует в GTCEu 1.21 — notConsumableItem
		.itemOutputs('megacells:printed_accumulation_processor')
		.duration(20)
		.circuit(1)
		.EUt(1920)

	// [PORT] ad_astra отсутствует в сборке 1.21.1 — лунный вариант рецепта удалён
	// event.recipes.gtceu.forming_press('megacells:printed_accumulation_processor_moon')
	//     .itemInputs('#c:dense_plates/silicon')
	...
	event.recipes.gtceu.circuit_assembler('ae2:accumulation_processor_moon')
		...
		// .inputFluids(Fluid.of('tfg:fluix', 144*5)) // [PORT-Ф2] tfg:fluix — кастомный GT-материал TFG, ещё не портирован
	...
	// 		'#gtceu:circuits/iv',
	// 		'1x minecraft:redstone')
	// 	.inputFluids(Fluid.of('tfg:cryogenized_fluix', 144*5)) // [PORT-Ф2]
	...
	// [PORT-Ф4] tfg:unfinished_inscriber_accumulation_press ещё не существует (фаза 4) — цепочка пресса закомментирована
	...
	//     .notConsumable('#c:lenses/black') // [PORT] forge: -> c:
	//     .itemOutputs('megacells:accumulation_processor_press')
	//     .duration(4000)
	//     .EUt(GTValues.VA[GTValues.IV])
	//     .cleanroom(CleanroomType.CLEANROOM)

	// [PORT-Ф4] tfg:unfinished_inscriber_accumulation_press (фаза 4); [PORT-Ф2] rocket_alloy_t1 / desh — кастомные материалы TFG
	// event.recipes.gtceu.implosion_compressor('tfg:unfinished_inscriber_accumulation_press')
	// 	.itemInputs(
	// 		'#c:dense_plates/rocket_alloy_t1',
	// 		'#c:dense_plates/desh',
	// 		'gtceu:quantum_star',
	...
	event.recipes.gtceu.shaped('megacells:mega_item_cell_housing', [
		...
		B: '#c:plates/maraging_steel_300', // [PORT] forge: -> c:
		C: '#c:fine_wires/niobium_titanium', // [PORT] forge: -> c:
		D: '#gtceu:circuits/iv',
	}).addMaterialInfo().id('tfg:crafting/mega_item_cell_housing')

	//Mega Fluid Cell Housing
	// [PORT-Ф2] incoloy_ma_956 — кастомный GT-материал TFG, ещё не портирован — рецепт закомментирован
	...
	// ], {
	//     A: 'ae2:quartz_vibrant_glass',
	//     B: '#c:plates/incoloy_ma_956',
	//     C: '#c:fine_wires/niobium_titanium',
	//     D: '#gtceu:circuits/iv',
	...
	//     .inputFluids(Fluid.of('tfg:cryogenized_fluix', 4608)) // [PORT-Ф2]
	...
	event.recipes.gtceu.assembly_line('megacells:cell_component_1m_256k')
		...
		// .inputFluids(Fluid.of('tfg:fluix', 4608)) // [PORT-Ф2] tfg:fluix — кастомный GT-материал TFG
		.inputFluids(Fluid.of('gtceu:polyphenylene_sulfide', 9216))
		.itemOutputs('megacells:cell_component_1m')
		.duration(4000)
	...
	//     .inputFluids(Fluid.of('tfg:cryogenized_fluix', 9216)) // [PORT-Ф2]
	...
	event.recipes.gtceu.assembly_line('megacells:cell_component_4m_256k')
		...
		// .inputFluids(Fluid.of('tfg:fluix', 9216)) // [PORT-Ф2] tfg:fluix — кастомный GT-материал TFG
		.inputFluids(Fluid.of('gtceu:polyphenylene_sulfide', 18432))
		.itemOutputs('megacells:cell_component_4m')
		.duration(4000)
	...
	event.recipes.gtceu.assembly_line('megacells:cell_component_16m')
		...
		// .inputFluids(Fluid.of('tfg:cryogenized_fluix', 18432)) // [PORT-Ф2] tfg-жидкость ещё не портирована
		// .inputFluids(Fluid.of('tfg:fluix', 18432)) // [PORT-Ф2]
		.inputFluids(Fluid.of('gtceu:polyphenylene_sulfide', 36864))
		.itemOutputs('megacells:cell_component_16m')
		.duration(8000)
		.EUt(GTValues.VA[GTValues.UV])
		// .dimension('ad_astra:moon') // [PORT] ad_astra отсутствует в сборке 1.21.1 (единственный рецепт — удалена только строка dimension)
		.cleanroom(CleanroomType.CLEANROOM)
		// [PORT-FIX] stationResearch ПОСЛЕ cleanroom: applyResearchProperty в GTM8 не инициализирует
		// список CONDITIONS (NPE, баг апстрима), а addCondition из cleanroom — инициализирует.
	...
	event.recipes.gtceu.assembly_line('megacells:cell_component_64m')
		...
		// .inputFluids(Fluid.of('tfg:cryogenized_fluix', 36864)) // [PORT-Ф2] tfg-жидкость ещё не портирована
		// .inputFluids(Fluid.of('tfg:fluix', 36864)) // [PORT-Ф2]
		...
		// .dimension('ad_astra:moon') // [PORT] ad_astra отсутствует в сборке 1.21.1 (единственный рецепт — удалена только строка dimension)
		.cleanroom(CleanroomType.CLEANROOM)
		// [PORT-FIX] stationResearch после cleanroom (см. выше)
	...
	event.recipes.gtceu.assembler('megacells:mega_crafting_unit')
		.itemInputs(
			...
			'#gtceu:circuits/luv',
			'6x #c:plates/titanium_tungsten_carbide', // [PORT] forge: -> c:
			'ae2:crafting_unit')
		// .inputFluids(Fluid.of('tfg:fluix', 144*8)) // [PORT-Ф2] tfg:fluix — кастомный GT-материал TFG
	...
	//         '4x ae2:engineering_processor',
	//         '#gtceu:circuits/luv',
	//         '6x #c:plates/titanium_tungsten_carbide',
	//         'ae2:crafting_unit')
	//     .inputFluids(Fluid.of('tfg:cryogenized_fluix', 144*4)) // [PORT-Ф2]
