...
| Ф10 | Космос (Ad Astra → Stellaris), ComputerCraft | 🔄 частично |
...
Оригинал — 270 модов, порт — 205: часть модов не существует на 1.21.1.
Ключевые замены:
...
- ComputerCraft: Tweaked — временно исключён (alpha-сборка под 1.21.1 крашит
  TFC при старте сервера; вернётся со стабильным релизом);
